...
def test_pick_highlights_multiple():
    items = [_item("v1", "C1"), _item("v2", "C2"), _item("v3", "C3")]
    picks = pick_highlights(items, [], limit=2)
    assert [p["videoId"] for p in picks] == ["v1", "v2"]


def test_pick_highlights_whitelist_only():
    items = [_item("v1", "UC_RANDOM"), _item("v2", "UC_FIFA_OFFICIAL"),
             _item("v4", "UC_BROADCASTER"), _item("v3", "UC_X")]
    picks = pick_highlights(items, ALLOW, limit=3)
    # 許可チャンネルのみ・優先順位順（公式→放送局）。許可外は除外
    assert [p["videoId"] for p in picks] == ["v2", "v4"]
...
def test_fetch_highlights_skips_cached_and_unplayed():
    ...
    existing = {"2026-06-11|Japan|Brazil": {"videos": [{"videoId": "cached"}]}}  # 既存はスキップ
    calls = []
    ...
    # 既存のJapan戦はそのまま、未消化は検索しない、Spain戦のみ新規検索（複数本）
    assert result["2026-06-11|Japan|Brazil"]["videos"][0]["videoId"] == "cached"
    assert result["2026-06-12|Spain|Italy"]["videos"][0]["videoId"] == "abc"
...
def test_main_writes_highlights_with_key(tmp_path):
    ...
    vids = data["items"]["2026-06-11|Japan|Brazil"]["videos"]
    assert vids[0]["videoId"] == "abc"
    assert "youtube.com/watch?v=abc" in vids[0]["url"]
