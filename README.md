...
### 勝敗予想オッズ（The Odds API）について
`ODDS_API_KEY` を登録すると、**未消化（これから行う）試合**のブックメーカー・オッズ
（h2h＝勝ち/引分/負け）を取得し、試合詳細に「勝敗予想の材料」として**平均オッズ＋勝率換算**
を表示する（控除率調整済み）。

- 取得元: The Odds API（the-odds-api.com）v4。無料 **500クレジット/月**（h2h・1地域で1コール＝1クレジット）。
- 登録: https://the-odds-api.com/ で無料登録 → APIキー → GitHub Secret `ODDS_API_KEY`。
- sport key は `soccer_fifa_world_cup`。履歴（過去試合）オッズは有料のため**未消化試合のみ**。
- **免責**: 表示は情報・予想材料であり賭博の推奨・斡旋ではない旨をページに明記。ブックメーカーへの
  リンクは張らない。鍵が無ければ非表示。
